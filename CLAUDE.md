...
- **情報階層リデザイン完了**: 7 → 5 セクション（後に Phase 5 で 6 セクション化）
...
- **サイドバーは 6 セクション**（今日の運営 / What-if・戦略 / 制度管理 / 退院調整 / 過去1年分析 / データ・設定）— Phase 5（2026-04-25）で「📈 過去1年分析」を独立化

### Phase 5（2026-04-25） — 経営者目線セクション分離
- 旧「🛡️ 制度管理 > 📊 過去1年分析」タブを新セクション「📈 過去1年分析」に独立化
- 制度管理 / 過去1年分析 の住み分けは **現場目線（今この瞬間の制度コンプライアンス監視） vs 経営者目線（過去 1 年の傾向理解、理事会・師長会議の議論材料）**
- 制度管理に残るタブ: 「🛡️ 制度・需要・C群」「💡 改善のヒント」（現場リアルタイム監視に絞る）
- 内部タブ構成は 1 タブ単一ページのまま（最小変更）
- 医師別分析タブ（データ・設定内）にデモデータ警告バナーを追加 — 上半分（A医師〜J医師）と下半分（実医師コード UEMH/TAM）の出典差を明示。実運用開始時に医師別分析の本物化が完了したら β 統合（医師別分析の過去CSV部を 📈 過去1年分析 に統合）を別 PR で実施予定
...
| v3.5m.2 | サイドバーの表示病棟切替を強調表示（2026-05-01 同日、PR #51）— 副院長指示「左サイドの表示病棟の切り替えの部分を目立たせて欲しい」に対応。「🏥 表示病棟（タップで切替）」の専用ボックス（左 4px accent ボーダー + 軽い影 + 角丸）を「パラメータ設定」直下に配置。下部に「📍 現在表示中：xxx」キャプションで選択状態を明示。頻繁な病棟切替が一目で把握・操作できるように | `bed_control_simulator_app.py` (サイドバー, L1745 周辺), `theme_css.py` (.bc-ward-switcher) |
