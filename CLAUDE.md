...
- **新機能:**
  ...
  - **施設基準チェック・需要波・C群コントロール（v3.2, 2026-04-11追加）**
    - 新規モジュール: `scripts/guardrail_engine.py`（施設基準チェックエンジン）、`scripts/demand_wave.py`（需要波モデル）、`scripts/c_group_control.py`（C群コントロール）
  ...
  - **救急搬送後患者割合15%管理（v3.3, 2026-04-11追加）**
    ...
    - 「あと何件必要か」の自動計算（残り必要件数＋残り診療日あたり必要件数の2値）、危険域での受入最優先モード提案
    - 下り搬送（救急患者連携搬送料算定）を分子に含む
    - C群コントロールとの連携: 救急搬送比率未達リスクをC群アラートに反映
    - 翌診療日朝の救急受入余力（`emergency_ratio.py`）: 退院予定・C群在院状況から空床予測
