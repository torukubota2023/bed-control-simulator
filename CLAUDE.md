...
- **新機能:**
  ...
  - **過去3ヶ月rolling 平均在院日数（v3.1, 2026年改定対応）** — 朝のブリーフィング・意思決定ダッシュボード・HOPE送信サマリーに常時表示。**施設基準は各病棟ごとに判定**（病院全体ではなく5F・6Fそれぞれが満たす必要あり）。データ不足時は揃っている日数で計算。関数: `calculate_rolling_los()` in `bed_data_manager.py`
- **将来構想（未実装）:**
