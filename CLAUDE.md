...
### [2026-04-20] 2025FY 入院経路データの 5 区分再エクスポート

**課題:** 現行の事務データ（`data/admissions_consolidated.csv` / `actual_admissions_2025fy.csv`）は `admission_type = 「当日(予定外/緊急) / 予定」` の **2 値のみ**。これでは制度上の「救急搬送後患者割合（救急車搬送+下り搬送のみ、15% 以上）」を厳密算出できない。

**依頼内容:**
- 2025FY（2025-04 〜 2026-03）の入院 1,965 件について、電子カルテから以下の **5 区分で再エクスポート** をお願いしたい：
  - 救急（救急車搬送）
  - 下り搬送（救急患者連携搬送料の算定分）
  - 外来紹介
  - 連携室
  - ウォークイン（自力来院）
- あわせて、**主傷病に対する手術の有無**（医科点数表 第二章第十部第一節の手術に限る）も各件に紐付けて出せると望ましい
- レセプト突合で救急医療管理加算 算定件数も取れれば、制度上の「救急搬送後」厳密値のクロスチェックが可能

**なぜ必要か:**
- `facility_criteria_actual_report_2025fy.md` の「5F 53.1% / 6F 61.1%」は **予定外入院割合** として正確だが、制度上の「救急搬送後 15%」とは定義差があり、理事会・プレゼンで倍率表現（3.5〜4.1 倍）を使えない状態になっている
- 2026-06-01 以降の本則完全適用＋改定 イ/ロ/ハ 区分判定のためにも、入院経路＋手術有無の 2 軸データが必須

**期限目安:** 次回理事会プレゼンまで（精査中表記のまま出すのは避けたい）
...
- [ ] **主傷病に対する手術の有無** フラグの入力欄（v4 `data_input_tab.py` — 追加予定）
- [ ] **イ/ロ/ハ 自動判定関数**（`scripts/reimbursement_config.py` — 追加予定）
- [ ] 2026 改定点数表（入院料1/2 × イ/ロ/ハ 6 区分）の定数化
- [ ] 過去/新規データの区別（`data_version` カラム：`legacy_binary` vs `detailed_v1`）
