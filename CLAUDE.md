...
## 🔴 進行中の課題（次回「再開です」で最初に取り組む）

### 週末空床コスト What-If シミュレーションの論理修正

**問題:** 改善のヒントタブの「週末空床コスト」What-If で、「金曜退院を木曜に前倒し」すると週末空床が自動的に減る前提で計算しているが、**これは論理的に間違い**。

**なぜ間違いか:** 退院を1日前倒ししても、その床に新しい入院が入らなければ、金曜夜の在院数は同じ → 週末空床も同じ。「前倒し = 空床削減」ではなく「前倒し + 空いた床を即日埋める行動」がセットで初めて意味がある。

**修正方針（ユーザー確定済み）:**
- スライダー1: 金曜退院の木曜前倒し人数
- スライダー2: 前倒しで空いた床に入院が入る確率（例: 50%）
- 実際に週末空床が減るのは「前倒し人数 × 充填確率」

**修正対象:**
- `scripts/bed_control_simulator_app.py` — 改善のヒントタブ内「📊 週末空床コスト」セクション
- 現在の `_bm_new_weekend_empty = max(0, _weekend_empty - _bm_shift)` が間違い → `_bm_shift × 充填確率` に修正
- デモシナリオ（bed_control_demo_scenario.docx）Scene 2 の入退院セット調整の説明も整合性確認
