...
### Python 仮想環境の用意 (Streamlit 実行に必要)

Playwright が `playwright.config.ts` の `webServer` で Streamlit を起動するため、`.venv` に Streamlit と必須ライブラリが入っている必要があります。

```bash
python3 -m venv .venv
.venv/bin/pip install streamlit plotly pandas numpy pytest
```

**重要**: `plotly` は `_summary_expander`（朝のサマリー）で必須です。未インストール時は ModuleNotFoundError によりダッシュボードの data-testid（occupancy / alos / phase / vacancy / action-card / guardrail-summary）がすべて消失し、E2E テストが全滅します。`pytest` は `tests/` 配下のユニットテスト用です。
...
所要時間は 1-2 分です。`node_modules/` フォルダが作成されます。`dotenv` と `@playwright/test` が入ります。
...
A: 原因は以下のいずれかです（発生頻度順）。

**(1) `.venv` に `plotly` 未インストール（最頻出）**
`bed_control_simulator_app.py` の朝のサマリー (`_summary_expander`) 内で `import plotly.graph_objects as go` が失敗すると、`with _summary_expander:` ブロック全体が描画されず、ダッシュボードの全 data-testid が DOM から消えます。
...
**(2) セクション遷移が反映されていない**
`alos` / `phase` / `guardrail-summary` はダッシュボードにはなく、サイドバーの「🎯 意思決定支援」「🛡️ 制度管理」セクションに配置されています。テストは `sidebar.locator('label:has-text("🎯 意思決定支援")').click()` で遷移し、Streamlit の遅延描画対策に 1500ms 追加待機する設計です（`playwright/test_app.spec.ts:87-112`）。

`page.getByRole('radio', { name: "..." }).click()` は使えません — Streamlit のラジオ `<input type="radio">` は `tabindex="-1"` で不可視のためクリック不能です。必ず親 `<label>` をクリックしてください。

**(3) UI 側で data-testid が壊れた**
`dashboard_view.py` / `guardrail_view.py` / `main_tab.py` で `data-testid="..."` 属性を削除・改名していないか確認します。

```bash
git diff scripts/views/ scripts/tabs/main_tab.py
```
