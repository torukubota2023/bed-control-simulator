...
| 平均在院日数 5F（推定） | 21 日以下（緩和時 22 日） | **16.5 日** | 🟢 |
| 平均在院日数 6F（推定） | 21 日以下（緩和時 22 日） | **15.6 日** | 🟢 |
...
- **85歳以上 20%** — 通年 **27.3%**（閾値 +7.3pt）→ **LOS +1日（22日）緩和の条件クリア**
- **平均在院日数 21 日（緩和時 22 日）** — 推定 5F 16.5 日 / 6F 15.6 日（稼働率 90% 仮定での近似値、退院データ不在のため実績確認推奨）
