...
// =============================================================================
{
  ...
  const bases = [
    ...
    { t: "施設基準実態レポート", b: "予定外入院率 5F 53.1% / 6F 61.1%（2 値分類、救急搬送後厳密値は精査中）、85+ 27.3% を理事会四半期報告", color: COLOR.navy },
  ...
  const headers = ["指標", "参考値/基準", "当院実績", "備考"];
  headers.forEach((h, i) => {
  ...
  // 2025FY データは「当日(予定外/緊急) / 予定」の 2 値分類のため、
  // 制度上の「救急搬送後（救急車搬送+下り搬送）」厳密値は 2026-07 頃に純実データで確定（手動シード入力で bridge 中）。
  // 現段階では「予定外入院割合」として参考表示する。
  const rows = [
    ["予定外入院 5F（通年・2値分類）", "参考値", "53.1%", "救急+他経路込み" ],
    ["予定外入院 6F（通年・2値分類）", "参考値", "61.1%", "救急+他経路込み" ],
    ["予定外入院 5F（rolling 3ヶ月）", "参考値", "54.4%", "救急+他経路込み" ],
    ["予定外入院 6F（rolling 3ヶ月）", "参考値", "60.1%", "救急+他経路込み" ],
...
// =============================================================================
{
  ...
  const qas1 = [
    ...
    },
    {
      ...
      a: "LOS・85+ 比率は安全マージンを持って達成。予定外入院率 5F 53.1% / 6F 61.1%（2 値分類のため制度上の救急搬送後厳密値は精査中だが、基準 15% 達成見込み）、85+ 27.3%、推定 LOS 5F 16.5 / 6F 15.6 日（緩和後 21 日に対し 4-5 日の余裕）。救急搬送後厳密値は2026-07 頃の純実データ蓄積後に確定（手動シード入力で bridge 中）。",
    },
    {
      q: "Q3: 短手 3 の戦略的増加余力はあるか？",
      a: "予定外入院率が 50-60% と高水準のため、制度上の救急搬送後厳密値（精査中）も基準 15% に対して十分な余裕がある見込み。現在月 22 名 → 月 30 名程度は技術的可能。ただし消化器内科・紹介元病院との段階的調整が前提。最終判断は事務の入院経路 5 区分再エクスポート後に制度上の救急搬送後値で行う。v4 では Day 5/6 境界の影響を可視化する仕組みを整備。",
    },
    {
