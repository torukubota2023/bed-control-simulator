...
単位系の注意:
- patient-days（延べ患者日数）: 入院患者数 × 在院日数の総和
- bed-days（延べベッド日数）: C群調整による追加/削減ベッド占有の延べ日数
- beds（ベッド数）: 1日あたりの実効ベッド数（bed-days / 日数 で換算）
- 日（days）: 平均在院日数の余力（LOS上限 - 現在LOS）
"""
...
C_REVENUE_PER_DAY = 33400       # C群 日次診療報酬（円/ベッド/日）
C_COST_PER_DAY = 4500           # C群 日次変動費（円/ベッド/日）
C_CONTRIBUTION_PER_DAY = 28900  # C群 1日あたり運営貢献額（円/ベッド/日）
...
def get_c_group_summary(
    ...
    # c_daily_contribution: C群患者数 × 1日あたり運営貢献額（円/日）
    return {
        "c_count": c_count,                    # C群患者数（人）
        "c_ratio": round(c_ratio, 1),          # C群患者割合（%）
        "total_patients": total_patients,      # 全入院患者数（人）
        "c_daily_contribution": c_count * C_CONTRIBUTION_PER_DAY,  # 円/日
...
def calculate_c_adjustment_capacity(
    ...
    # --- 余力の計算 ---
    # headroom_days: LOS上限と現在LOSの差（日）。これは「1患者あたり」の余力
    headroom_days = guardrail_los_limit - current_los

    # headroom_patient_days: headroom_daysをrolling window全体の延べ患者日数に換算
    # = headroom_days × (入退院数の平均) / 実測日数
    # 単位: patient-days（延べ患者日数）
    ...
    n_denominator = (total_admissions + total_discharges) / 2  # rolling windowの分母（人）
    ...
    # max_delay_bed_days: C群退院を後ろ倒しできる延べベッド日数（bed-days）
    ...
    return {
        "current_los": round(current_los, 2),         # 現在の平均在院日数（日）
        "los_limit": guardrail_los_limit,              # 制度上の平均在院日数上限（日）
        "headroom_days": round(headroom_days, 2),      # LOS余力（日）= 上限 - 現在LOS
        "headroom_patient_days": round(headroom_patient_days, 1),  # LOS余力の延べ患者日数換算（patient-days）
        "can_delay_discharge": can_delay,
        "max_delay_bed_days": round(max_delay_bed_days, 1),  # 後ろ倒し可能な延べベッド日数（bed-days）
        "can_accelerate_discharge": True,  # 前倒し退院は常に可能
        "delay_revenue_per_day": C_CONTRIBUTION_PER_DAY,  # 円/ベッド/日
        "status": status,
...
def simulate_c_group_scenario(
    ...
    # LOS変動の計算
    # delta_patient_days: 退院タイミング変更による延べ患者日数の増減（patient-days）
    # 後ろ倒し: +n_delay × delay_days, 前倒し: -n_accelerate × accelerate_days
    ...
    # 収益影響の計算
    # total_extra_patient_days: 追加（or 削減）延べベッド日数（bed-days）
    # revenue_impact_total: total_extra_patient_days × 1日あたり運営貢献額（円）
    total_extra_patient_days = (n_delay * delay_days) - (n_accelerate * accelerate_days)
    revenue_impact_total = total_extra_patient_days * C_CONTRIBUTION_PER_DAY
    revenue_impact_daily_avg = revenue_impact_total / 30  # 月平均の日次換算（円/日）
    # 後方互換のために旧キーも維持
    revenue_impact_daily = revenue_impact_daily_avg
    revenue_impact_monthly = revenue_impact_total
    ...
    description = (
        ...
        f"収益影響: 合計 {revenue_impact_total:+,.0f}円（1日平均 {revenue_impact_daily_avg:+,.0f}円）"
    )
    ...
    return {
        ...
        "revenue_impact_daily_avg": revenue_impact_daily_avg,
...
def calculate_demand_absorption(
    ...
    Returns:
        需要吸収計算結果 dict。
    """
    demand_gap = target_occupancy - occupancy_rate  # 正なら稼働率不足（小数）
    # demand_gap_beds: 目標との差をベッド数に換算（ベッド数）
    demand_gap_beds = demand_gap * TOTAL_BEDS

    max_delay_bed_days = c_adjustment_capacity.get("max_delay_bed_days", 0.0)

    # max_delay_bed_days（延べベッド日数）を7日で割り、1日あたりの調整可能ベッド数に変換
