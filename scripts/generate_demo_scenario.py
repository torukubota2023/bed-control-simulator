...
# ===== Scene 8 =====
add_scene_heading("■ Scene 8　全体主義ベッドコントロール —「うちの病棟さえ良ければ」を超える　(3分)")

add_serif("💬 ここまで病棟別に課題を見てきました。5Fは稼働率が低め、6Fは満床に近い。普通ならこう思いますよね——「5Fはもっと頑張れ」「6Fは問題ない」と。")

add_empty_line()

add_serif("💬 でも、ちょっと視点を変えてみましょう。")
...
add_serif("💬 このアプリには「全体主義ベッドコントロール」という考え方を組み込みました。")
...
# Narration: show the multi-stage table
add_serif("💬 アプリでは、この関係が自動的に検出されます。")

add_serif("💬 「ヘルプ病棟」＝支援が必要な病棟。「ヘルパー病棟」＝支える側の病棟。月ごとの実績に基づいて、役割は自動的に切り替わります。")
...
p = doc.add_paragraph()
p.paragraph_format.space_after = Pt(8)
run = p.add_run("【操作】6Fを選択 → 稼働率推移グラフの下に多段階テーブルが表示される")
set_font(run, size=Pt(11), color=NAVY, bold=True)

add_empty_line()

add_serif("💬 6Fのグラフを見てください。青い点線が見えますね。これは「全体目標を達成するために6Fが維持すべき稼働率」です。自分の病棟だけの目標ではなく、5Fと合わせた全体目標から逆算しています。")

add_empty_line()

add_serif("💬 そしてグラフの下に、多段階のシナリオテーブルが出ています。")

add_empty_line()

# Multi-stage table example
_holistic_table = doc.add_table(rows=5, cols=3)
...
_htW = parse_xml(f'<w:tblW {nsdecls("w")} w:type="pct" w:w="4000"/>')
_htPr.append(_htW)
...
for r_idx, row_data in enumerate(_holistic_data):
    for c_idx, cell_text in enumerate(row_data):
        ...
        is_header = (r_idx == 0)
        is_current = (r_idx == 3)
        _ht_color = NAVY if is_header else RED if is_current else DARK_BLUE
        _ht_bold = is_header or is_current
...
add_serif_with_highlight([
    ("💬 これが「全体主義」の考え方です。「うちの病棟さえ良ければいい」ではなく、", {}),
    ("「全体で目標を達成する」", {'color': RED, 'bold': True}),
    ("。月によっては5Fがヘルパーになることもあります。助け合いの関係を、データで見えるようにする。", {}),
])

add_empty_line()

add_serif("💬 ちなみに、両方の病棟が苦しい月もあります。その場合は全体主義の表示は出ません。「まず自分の病棟を立て直す」ことに集中するべきだからです。")
...
add_serif("💬 4つ目。病棟間の全体主義ベッドコントロール。「うちさえ良ければ」ではなく、全体で目標を達成する道筋が見える。")
...
_features = [
    ...
    ["🤝 全体主義ベッドコントロール", "病棟間の助け合いを数値化。ヘルパー/ヘルプ病棟の自動判定と多段階シナリオ表示"],
