...
add_serif("💬 ここで「本当に公平なのか？」という疑問が出るかもしれません。前半で低かった病棟と、頑張っていた病棟が同じ+1.6ポイント——前半の差を無視していないか、と。")

add_empty_line()

add_serif("💬 大事なポイントが3つあります。")

add_empty_line()

add_serif_with_highlight([
    ("💬 1つ目。均等努力は", {}),
    ("過去の評価ではなく、未来の行動計画", {'bold': True}),
    ("です。前半の振り返りは月次レビューで行います。でも今、残り10日で大事なのは「何ができるか」。そこに両病棟が同じ力を出す——これが一番シンプルで、一番動ける目標設定です。", {}),
])
...
add_serif_with_highlight([
    ("💬 2つ目。同じ+1.6ポイントでも、", {}),
    ("必要なアクションは病棟ごとに違います", {'bold': True}),
    ("。87%の5Fは入院を増やす「攻め」の施策が要る。92%の6Fは退院調整で「守る」施策で済む場合もある。同じ数字でも、実は中身が違うんです。", {}),
])

add_empty_line()

add_serif("💬 3つ目。もし前半の不足を5Fだけに背負わせると、残り10日で+3ポイント以上が必要になります。物理的に達成不能な目標は、行動を生みません。達成可能な目標を両病棟に渡すことが、チームとしての成果につながります。")

add_empty_line()

add_serif_with_highlight([
    ("💬 つまり均等努力とは——", {}),
    ("「犯人探し」ではなく「チームとして達成する」ための仕組み", {'color': RED, 'bold': True}),
    ("です。", {}),
])
