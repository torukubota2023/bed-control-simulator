...
# ===== Scene 3 =====
add_scene_heading("■ Scene 3　「平均在院日数 21.7日」— 算定基準を超えています　(3分)")

add_operation("▶ 「運営分析」タブに戻る")
add_operation("▶ 下段のメトリクスカード「平均在院日数 21.7日」の赤いdelta表示を見せる")

add_serif_with_highlight([
    ("💬 ここを見てください。平均在院日数 ", {}),
    ("21.7日", {'color': RED, 'bold': True}),
    ("。赤い矢印で「基準超過 ", {}),
    ("+0.7日", {'color': RED, 'bold': True}),
    ("」と出ています。", {}),
])
...
add_serif_with_highlight([
    ...
    ("+1.7ポイント", {'color': RED, 'bold': True}),
    ("ずつ上げればいい。5Fは88.7%へ、6Fは93.7%へ。全体で約90%達成です。", {}),
])
...
_holistic_data = [
    ...
    ["+0.5pt", "87.5%", "92.5%", "89.0%", "❌"],
    ["+1.0pt", "88.0%", "93.0%", "89.5%", "❌"],
    ["+1.7pt", "88.7%", "93.7%", "90.0%", "✅"],
    ["+2.0pt", "89.0%", "94.0%", "90.5%", "✅"],
    ["+4.0pt", "91.0%", "96.0%", "92.5%", "⚠️（96%上限）"],
]
for r_idx, row_data in enumerate(_holistic_data):
    for c_idx, cell_text in enumerate(row_data):
        ...
        is_target = (r_idx == 3)
        is_warning = (r_idx == 5)
        _ht_color = NAVY if is_header else GREEN if is_target else RED if is_warning else DARK_BLUE
...
add_serif_with_highlight([
    ("💬 +1.7ポイントずつ——これが今月の「均等努力ライン」です。テーブルの", {}),
...
add_serif("💬 下の行を見てください。Δが大きすぎると、片方が96%を超えてしまう。これは現実的に無理なシナリオです。アプリでは96%上限を超えるケースに警告マークが付きます。「そこまでは求めない」というラインが見えるわけです。")
