...
計算対象:
    ...
    (b) 平均在院日数（制度上限 21 日、85歳以上割合 20% 以上で 22 日に緩和）
...
LOS_BASELINE_DAYS: float = 21.0  # 平均在院日数の制度上限
LOS_RELAXED_DAYS: float = 22.0  # 85歳以上割合≧20%で適用される緩和上限
...
| 平均在院日数 5F（推定） | 21 日以下（緩和時 22 日） | **{los_5f_avg:.1f} 日** | {'🟢' if los_5f_avg <= los_threshold else '🟡'} |
| 平均在院日数 6F（推定） | 21 日以下（緩和時 22 日） | **{los_6f_avg:.1f} 日** | {'🟢' if los_6f_avg <= los_threshold else '🟡'} |
...
- **85歳以上 20%** — 通年 **{eld['elderly_85_pct']:.1f}%**（閾値 +{eld['elderly_85_pct']-ELDERLY_85_THRESHOLD_PCT:.1f}pt）→ **LOS +1日（22日）緩和の条件クリア**
- **平均在院日数 21 日（緩和時 22 日）** — 推定 5F {los_5f_avg:.1f} 日 / 6F {los_6f_avg:.1f} 日（稼働率 90% 仮定での近似値、退院データ不在のため実績確認推奨）
...
- 平均在院日数 **21 日以下**
- ただし **85歳以上割合が 20% 以上**の場合は **22 日以下**に緩和
...
- 85歳以上の入院患者割合が **20% 以上** で、平均在院日数の上限が 21 → **22 日に緩和**
