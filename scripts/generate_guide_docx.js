...
// ─── Config ───
const INPUT = "/Users/torukubota/ai-management/docs/admin/hospital_app_development_guide_v2.md";
const OUTPUT = "/Users/torukubota/ai-management/docs/admin/hospital_app_development_guide_v2.docx";
...
// ─── Preprocessing: unescape markdown characters ───
function unescapeMarkdown(content) {
  // Replace escaped characters with their unescaped equivalents
  // Must process multi-char escapes before single-char ones
  // \-\-- → --- (horizontal rule / em dash separator)
...
function buildTable(headerRow, dataRows) {
  ...
  for (const row of dataRows) {
    ...
    rows.push(
      new TableRow({
        children: row.slice(0, numCols).map((c) => makeCell(c, false)),
      })
    );
  }
...
async function main() {
  ...
  // Preprocess: unescape markdown characters
