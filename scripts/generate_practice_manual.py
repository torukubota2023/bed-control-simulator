...
def generate_manual():
    ...
    run3 = subtitle_p.add_run("地域包括医療病棟の経営最適化のために")
    ...
    add_body(doc,
        ...
        "収益とコストのバランスを最適化するための経営判断支援ツールです。")
    add_body(doc,
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：検査・初期治療で変動費が高く、限界利益は3群中最小",
        "経営的意味：必要なフェーズだが、A群ばかりだと変動費増で限界利益が相対的に低下",
    ])
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：治療が安定し、変動費が大幅に下がるが収益は維持。最も限界利益を生むフェーズ",
        "経営的意味：B群を厚くすることが限界利益最大化の鍵",
    ])
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：変動費最小で限界利益は高い。ただし長期化すると新規入院を阻害（機会損失）",
        "経営的意味：需給の調整弁。閑散期は長めに保持、繁忙期は早めに退院促進",
    ])

    # フェーズ比較表
    doc.add_heading("フェーズ比較一覧", level=3)
    add_styled_table(doc,
        ["フェーズ", "入院日数", "日次収益", "日次変動費", "日次限界利益", "経営的役割"],
        [
            ["A群", "1〜5日目", "36,000円", "12,000円", "24,000円", "初期投資（必要コスト）"],
            ["B群", "6〜14日目", "36,000円", "6,000円", "30,000円", "限界利益の源泉（最重要）"],
            ["C群", "15日目〜", "33,400円", "4,500円", "28,900円", "需給調整弁"],
        ])

    # 1-4
    doc.add_heading("1-4. 包括払いの前提", level=2)
    add_body(doc,
        "地域包括医療病棟は包括払い制度です。日次収益は入院14日目まで約36,000円/日、15日目以降は約33,400円/日です。")
    add_body(doc,
        "変動費は初期（検査・処置）に集中するため、入院後半ほど限界利益が大きくなります。"
        "A群（初期）は変動費が高く限界利益は3群中最小ですが、B群・C群になるにつれて限界利益が拡大します。")
    add_body(doc,
        "ただし、C群を長期保持すると新規入院の機会損失が発生するため、"
        "「いつ退院させるか」が経営判断の核心となります。", bold=True)

    # 1-5
    doc.add_heading("1-5. このシミュレーターでできること", level=2)
    add_body_list(doc, [
        "3つの戦略（回転重視・安定維持・バランス）で病棟運営をシミュレーション",
        "30日間の日次推移を可視化（稼働率・在院患者数・入退院数・限界利益）",
        "月次の収益・変動費・限界利益を算出",
        "A/B/Cフェーズ構成比の時系列変化を確認",
        "経営判断フラグ（稼働率低下、A群過多、C群滞留など）を自動検知",
        "3戦略の一括比較で最適戦略を特定",
    ...
    add_body(doc, "本ツールは経営判断の参考情報であり、以下の限界があります。", bold=True)
    add_body_list(doc, [
        "診療報酬の厳密な計算ではない（経営判断のための簡易モデル）",
        "個別患者の重症度・疾患は考慮していない",
        "看護必要度の算定への影響は未反映",
        "緊急入院と予定入院の区別なし",
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["月次限界利益", "約3,062万円", "約3,445万円", "約3,360万円"],
            ["B群比率", "約49%", "約33%", "約40%"],
            ["C群比率", "約18%", "約27%", "約23%"],
    ...
    add_indent_block(doc, "→ 限界利益最大化 → 安定維持戦略")
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["機会損失コスト", "10,000円", "満床で入院を断った場合の空床1日あたり逸失利益"],
    ...
    add_body_list(doc, [
        ...
        "日次限界利益：棒グラフで日々の限界利益。安定して高いのが理想",
    ])

    doc.add_heading("タブ2: フェーズ構成", level=3)
    add_body_list(doc, [
        "A/B/C積み上げ面グラフ：赤(A群)・緑(B群)・青(C群)の構成比の時系列変化。緑（B群）が厚いほど限界利益が良い",
        "平均円グラフ：月間平均の構成比。一目で全体バランスを把握",
        "フェーズ別患者数：各群の実数推移",
    ])

    doc.add_heading("タブ3: 収支分析", level=3)
    add_body_list(doc, [
        "メトリクスカード：月次収益・変動費・限界利益・平均稼働率などの主要指標",
        "日次収益・変動費推移：収益と変動費の乖離が大きいほど限界利益が大きい",
        "累積限界利益推移：右肩上がりの角度が急なほど効率が良い",
    ])

    doc.add_heading("タブ4: 経営判断フラグ", level=3)
    add_body_list(doc, [
        "各日のフラグ一覧（赤=警告、緑=正常）",
    ...
    p = doc.add_paragraph(style="List Number")
    p.clear()
    p.add_run("月次限界利益と目標レンジ日数を確認する").font.size = Pt(10.5)
    ...
    add_body(doc, "注目点：回転重視戦略の機会損失が最も少ないか確認する。")
    ...
    p = doc.add_paragraph(style="List Number")
    p.clear()
    p.add_run("実行して収支分析タブを確認する").font.size = Pt(10.5)
    add_body(doc, "注目点：回転重視戦略の限界利益が改善されるか確認する（入院初期の収益が増えるため）。")
    ...
    # 第5章：結果の読み方と経営判断への活用
    # ================================================================
    doc.add_heading("第5章　結果の読み方と経営判断への活用", level=1)
    ...
    add_styled_table(doc,
        ["優先順位", "指標", "意味", "目標"],
        [
            ["1", "月次限界利益", "経営の最終目標", "3,000万円以上"],
            ["2", "目標レンジ日数", "運営の安定性", "20日以上/30日"],
            ["3", "B群構成比", "限界利益の源泉", "40〜50%"],
            ["4", "機会損失", "見えないコスト", "できるだけ少なく"],
        ])
    ...
    doc.add_heading("日次限界利益", level=3)
    add_body(doc,
    ...
    # 5-3
    doc.add_heading("5-3. 経営判断フラグの活用", level=2)
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["日次赤字", "日次限界利益がマイナス", "コスト構造の見直し"],
        ])

    # 5-4
    doc.add_heading("5-4. 月次レポートへの活用例", level=2)
    add_body(doc, "毎月の経営会議で以下を報告することを推奨します。", bold=True)
    p = doc.add_paragraph(style="List Number")
    p.clear()
    ...
    qa_list = [
        ...
        ("Q: 実際の診療報酬と金額が違うのですが？",
         "A: このシミュレーターは経営判断用の簡易モデルです。実際の診療報酬を入力したい場合は、「患者フェーズ別パラメータ」の収益・コストを実態に合わせて調整してください。"),
        ("Q: 94床以外の病棟でも使えますか？",
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["限界利益", "収益 - 変動費。人件費等の固定費は空床でも発生するため除外した利益"],
            ["機会損失", "空床や満床で入院を受け入れられなかった場合の逸失利益。約25,000円/空床/日"],
            ["フェーズ構成比", "A/B/C各群の患者数が全体に占める割合。B群40〜50%が理想"],
            ["包括払い", "入院期間を通じて1日あたりの報酬が定額で支払われる制度"],
            ["在院日数", "入院してからの経過日数。フェーズ分類の基準"],
            ["需給調整弁", "C群患者を退院タイミングの調整に活用する考え方。閑散期は保持、繁忙期は退院促進"],
    ...
    add_body_list(doc, [
        ...
        "PNGファイル：稼働率推移・フェーズ構成比・収支分析のグラフ",
    ])
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["avg_length_of_stay", "平均在院日数", "18", "10〜30", "日"],
    ...
    add_styled_table(doc,
        ["変数名", "説明", "初期値", "単位"],
        [
            ["phase_a_revenue", "A群 日次収益", "30,000", "円/日/人"],
            ["phase_a_cost", "A群 日次コスト", "28,000", "円/日/人"],
            ["phase_b_revenue", "B群 日次収益", "30,000", "円/日/人"],
            ["phase_b_cost", "B群 日次コスト", "13,000", "円/日/人"],
            ["phase_c_revenue", "C群 日次収益", "30,000", "円/日/人"],
    ...
    add_styled_table(doc,
        ["変数名", "説明", "初期値", "範囲", "単位"],
        [
            ["opportunity_cost", "機会損失コスト", "10,000", "0〜50,000", "円/空床/日"],
