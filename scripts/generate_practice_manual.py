...
def generate_manual():
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：検査・初期治療でコストが高く、粗利はほとんど出ない",
        "経営的意味：必要なフェーズだが、A群ばかりだとコスト増で粗利悪化",
    ])
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：治療が安定し、コストが下がるが収益は一定。最も利益を生むフェーズ",
        "経営的意味：B群を厚くすることが粗利最大化の鍵",
    ])
    ...
    add_body(doc, "")
    add_body_list(doc, [
        "特徴：コスト最小で粗利は高い。ただし長期化すると新規入院を阻害（機会損失）",
    ...
    add_styled_table(doc,
        ["フェーズ", "入院日数", "日次収益", "日次コスト", "日次粗利", "経営的役割"],
        [
            ["A群", "1〜5日目", "30,000円", "28,000円", "2,000円", "初期投資（必要コスト）"],
            ["B群", "6〜14日目", "30,000円", "13,000円", "17,000円", "粗利の源泉（最重要）"],
            ["C群", "15日目〜", "30,000円", "11,000円", "19,000円", "需給調整弁"],
        ])

    # 1-4
    doc.add_heading("1-4. 包括払いの前提", level=2)
    add_body(doc,
        "地域包括医療病棟は包括払い制度です。日次収益は入院期間を通じて一定（30,000円/日と仮定）。")
    add_body(doc,
        "コストは初期（検査・処置）に集中するため、入院後半ほど粗利が大きくなります。"
        "このため、A群（初期）の粗利は極めて小さく、B群・C群になるにつれて粗利が拡大します。")
    add_body(doc,
    ...
    add_body_list(doc, [
        ...
        "30日間の日次推移を可視化（稼働率・在院患者数・入退院数・粗利）",
        "月次の収益・コスト・粗利を算出",
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["月次粗利", "約3,062万円", "約3,445万円", "約3,360万円"],
            ["B群比率", "約49%", "約33%", "約40%"],
            ["C群比率", "約18%", "約27%", "約23%"],
    ...
    add_indent_block(doc, "→ 粗利最大化 → 安定維持戦略")
    ...
    add_styled_table(doc,
        ["パラメータ", "A群", "B群", "C群"],
        [
            ["日次収益", "30,000円", "30,000円", "30,000円"],
            ["日次コスト", "28,000円", "13,000円", "11,000円"],
        ])
    ...
    add_body_list(doc, [
        ...
        "日次粗利：棒グラフで日々の粗利。安定して高いのが理想",
    ])

    doc.add_heading("タブ2: フェーズ構成", level=3)
    add_body_list(doc, [
        "A/B/C積み上げ面グラフ：赤(A群)・緑(B群)・青(C群)の構成比の時系列変化。緑（B群）が厚いほど粗利が良い",
    ...
    add_body_list(doc, [
        "メトリクスカード：月次収益・コスト・粗利・平均稼働率などの主要指標",
        "日次収益・コスト推移：収益とコストの乖離が大きいほど粗利が大きい",
        "累積粗利推移：右肩上がりの角度が急なほど効率が良い",
    ])
    ...
    p = doc.add_paragraph(style="List Number")
    p.clear()
    p.add_run("月次粗利と目標レンジ日数を確認する").font.size = Pt(10.5)
    ...
    add_body(doc, "注目点：回転重視戦略の粗利が改善されるか確認する（入院初期の収益が増えるため）。")
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["粗利（あらり）", "収益 - コスト。人件費等の固定費を含まない利益"],
            ["機会損失", "満床で入院を受け入れられなかった場合の逸失利益。1件あたり10,000円で計算"],
