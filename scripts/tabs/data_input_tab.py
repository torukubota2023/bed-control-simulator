...
def _render_detail_input():
    ...
    with st.form("detail_entry_form", clear_on_submit=True):
        ...
        if st.form_submit_button("追加"):
            ...
            new_row = pd.DataFrame([{
                ...
                "route": ev_route if ev_type_en == "admission" else "",
                "los_days": ev_los if ev_type_en == "discharge" else None,
                "日付": str(ev_date),
                "病棟": ev_ward,
                "入退院区分": ev_type_ja,
                "経路": ev_route if ev_type_en == "admission" else "",
            }])
    ...
    if current is not None and len(current) > 0:
        st.markdown("---")
        st.markdown("##### 現在のデータ")
        display_cols = [c for c in ["date", "ward", "event_type", "route", "los_days"] if c in current.columns]
